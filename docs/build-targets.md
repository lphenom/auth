...
| `CodeSenderInterface` (email) | `HttpEmailSender` (**shared,kphp**) | `file_get_contents` + HTTP context — KPHP-совместимо |
...
- Совместим с Mailgun, SendGrid, Mailersend, любым кастомным HTTPS-эндпоинтом
...
$emailSender = new \LPhenom\Auth\Support\EmailSender\HttpEmailSender(
    ...
    $_ENV['EMAIL_API_KEY'],
    $_ENV['EMAIL_FROM'],
    $_ENV['EMAIL_SUBJECT'] ?? 'Ваш код подтверждения'
);
...
# HTTP Email API (Mailgun, SendGrid, Mailersend, кастомный HTTPS POST)
EMAIL_API_URL=https://api.mailgun.net/v3/example.com/messages
EMAIL_API_KEY=key-xxxxxxxxxxxxxxx
EMAIL_FROM=noreply@example.com
